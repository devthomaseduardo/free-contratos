...
import { FileText, Calculator, UserCircle, ScrollText, ShieldCheck, ChevronRight, Receipt, FileBadge, Lock, Send } from 'lucide-react';
    ...
    <div className="w-20 lg:w-72 bg-[#0B0F19] border-r border-slate-800 flex flex-col h-full shrink-0 transition-all duration-300">
      ...
      <div className="h-20 flex items-center justify-center lg:justify-start lg:px-8 border-b border-slate-800/50">
        ...
        <div className="ml-4 hidden lg:block">
            <h1 className="font-bold text-lg text-white tracking-tight leading-none">Contract<span className="text-indigo-400">Forge</span></h1>
      ...
      <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
        ...
        {menu.map((item) => {
          ...
          return (
            <button
              key={item.id}
              onClick={() => onSelect(item.id as DocumentType)}
            ...
            >
              ...
              {isActive && <ChevronRight size={16} className="text-slate-500 hidden lg:block animate-pulse" />}
            </button>
          );
        })}
      </nav>
